...
void main()
{
    float process[500],aTime[500],bTime[500],ctTime[500],wTime[500],tat_time[500];
    int n = 0,i = 0 ;
    ...
    wTime[0] = 0;
    tat_time[0] = bTime[0];
    ctTime[0] = bTime[0]+aTime[0];
    for( i = 1 ; i < n ; i++)
	{
        ctTime[i] = ctTime[i-1] + bTime[i];
        tat_time[i] = ctTime[i] - aTime[i];
    ...
    for(i = 0 ; i < n ; i++){
        printf("P[%d]\t%.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n",i,aTime[i],bTime[i],ctTime[i],tat_time[i],wTime[i]);
    }
